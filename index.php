...
<?php
    if(!empty($_GET['test'])){
        ...
        include_once('bucky-box-api-wrapper.php');
        $api = new BuckyBoxApiWrapper($api_key, $api_secret);
        ...
        switch ($test) {
            case 'boxes':
                ...
                break;
            case 'box':
                $code = $api->boxes()->id(12, ["embed"=>"extras,images"]);
                break;
            case 'customers':
                $code = $api->customers()->all(["embed"=>"address"]);
                break;
            case 'customer':
                $code = $api->customers()->id(123,["embed"=>"address"]);
                break;
            case 'customernew':
                $code = $api->customers()->create('{"customer": {"first_name": "Joe","last_name": "Smith","email": "joe@buckybox.com", "delivery_service_id": 146, "address": {
